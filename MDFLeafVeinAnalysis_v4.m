...
% calculate the ratio of the MST to the total length
...
nL = 1;
% set up an array for the new EndNodes
...
V = fnc_summary_veins(G_veins,total_area,polygon_area,micron_per_pixel);
HLD_metrics(1,:) = [V A P];
% get all the current PixelIdxList at the start as a cell array;
P_PIL = {polygon_stats.PixelIdxList};
% set up the endnodes
EndNodes = zeros(nnP*2-2,2,'single');
% % set up a dummy value to dimension the stats arrays
% polygon_stats(numel(criteria)+1) = polygon_stats(1);
% areole_stats(numel(criteria)+1) = areole_stats(1);
...
for iE = 1:neP
    ...
    if Ni~=Nj
        ...
        % delete any edges in the vein graph upto and including this edge width
        ...
        % add the pixelidxlist for the nodes to fuse to the new node
        P_PIL(Nk) = {cat(1,P_PIL{Ni},P_PIL{Nj})};
        %         % summarise the vein graph
        %         xxx =
        %         % extract metrics from the remaining network
        %         wEdges(Nf,1) = Wij;
        %         nEdges(Nf,1) = numedges(G_veins);
        %         nNodes(Nf,1) = numnodes(G_veins);
        %         total_length(Nf,1) = sum(G_veins.Edges.Length);
        %         total_volume(Nf,1) = sum(G_veins.Edges.Volume);
        %         % calculate the number of FEV created
        %         D = degree(G_veins);
        %         FEV(Nf,1) = sum(D==1);
        switch HLD_level_criteria
            ...
            case {'width';'log width';'sqrt width';'sq width';'number'}
                ...
                if iE >= criteria(nL)
                    ...
                    % NOTE THIS IS CALCULATING THE STATS ON THE COMPLETE
                    % REGION IMAGE NOT JUST THE NEW REGION
                    PCC.NumObjects = length(include);
                    PCC.PixelIdxList  = P_PIL(include);
                    LM = labelmatrix(PCC);
                    % calculate all the metrics for the new fused region
                    [A_stats,P_stats] = fnc_polygon_analysis(bw_polygons,bw_areoles, LM);
                    %                     if ~isempty(A_stats)
                    %                         areole_stats(nL)
                    %                         areole_stats(nL) = A_stats;
                    %                     end
                    %                     polygon_stats(nL) = P_stats;
